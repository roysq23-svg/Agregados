...
export default function HistorialViajes() {
  ...
  const dineroCobrado = viajes.reduce((acc, v) => {
    ...
    return (estado === 'pagado' || estado === 'pagado completo') ? acc + (parseFloat(v.monto_total) || 0) : acc;
  }, 0);
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .val-pago { font-weight: bold; color: #10b981; }
          ...
          <div class="ticket-container">
            ...
            <table class="datos-table">
              ...
              <tr>
                ...
                <td class="val-pago">${viaje.estado_pago.toUpperCase()}</td>
                <td class="lbl">REG.:</td>
                <td style="text-transform: lowercase;">${profile.usuario || 'carlos'}</td>
              </tr>
            </table>
            ...
            <div class="footer-container">
              <table class="totales-table">
                <tr>
                  ...
                  <td>S/ ${viaje.monto_total.toFixed(2)}</td>
                </tr>
                <tr>
                  <td class="lbl-t" style="color: #10b981;">COBRADO</td>
                  <td style="color: #10b981; font-weight: bold;">S/ ${viaje.estado_pago === 'pagado' ? viaje.monto_total.toFixed(2) : '0.00'}</td>
                </tr>
                <tr>
                  <td class="lbl-t">SALDO PEND.</td>
                  <td>S/ ${viaje.estado_pago === 'pendiente' ? viaje.monto_total.toFixed(2) : '0.00'}</td>
                </tr>
                <tr class="total-row">
  ...
  return (
    // CAMBIO DE ENTORNO: Pasamos de bg-slate-900 a un fondo claro descansado
    <div className="min-h-screen bg-slate-50 text-slate-800 p-4 pb-20">
      <div className="max-w-md mx-auto space-y-4">
        
        {/* FILTRO DE FECHA MODO CLARO */}
        ...
        {/* BILLETERA PERSONAL DEL CHOFER EN CLARO PREMIUM */}
        {!loading && (viajes.length > 0 || gastos.length > 0) && (
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl shadow-sm">
              <span className="text-[9px] uppercase font-bold text-amber-700 block">Clientes por Cobrar</span>
              <span className="text-base font-black text-amber-900 font-mono block mt-0.5">S/ {deudasPorCobrar.toFixed(2)}</span>
              <span className="text-[9px] text-slate-400 block mt-0.5">Dinero pendiente en ruta</span>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            {/* SUB-BLOQUE: RENDICIÓN DE MIS GASTOS EN RUTA */}
            {gastos.length > 0 && (
              ...
              {viajes.length === 0 ? (
                ...
                viajes.map((viaje) => (
                  <div key={viaje.id} className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-sm space-y-3">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`text-[9px] px-2 py-0.5 rounded font-bold uppercase border ${
                          viaje.estado_pago?.toLowerCase().trim() === 'pagado' || viaje.estado_pago?.toLowerCase().trim() === 'pagado completo'
                    ...
                    {/* DESGLOSE INTERNO DE MATERIALES */}
                    ...
                    {/* BOTÓN IMPRIMIR TOTALMENTE REFINADO */}
                    <button 
